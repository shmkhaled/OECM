...
//    targetClassesWithoutURIs.foreach(println(_))
    val targetRelationsWithoutURIs = ontStat.RetrieveRelationsWithoutURIs(targetOntology)
    println("All relations in the target ontology are: "+targetRelationsWithoutURIs.count())
    targetRelationsWithoutURIs.foreach(println(_))
    val sourceClassesAndRelationsWithURIs = ontStat.RetrieveClassesWithURIsAndLabels(sourceOntology) //applied for ontologies with codes like Multifarm ontologies
...
//    sourceOntologyLabels.foreach(println(_))
    ...
    val sourceRelations: RDD[(String, String)] = ontStat.RetrieveRelationsWithURIs(sourceLabelBroadcasting,sourceOntology)
...
//    availableTranslations.foreach(println(_))
    val sourceClassesWithListOfBestTranslations = availableTranslations.map(x => (x._1,x._2,translate.GetBestTranslationForClass(x._2,targetClassesBroadcasting)))//.cache()
...
//    ####################### Relation Translation #######################
    val relationsWithTranslation = translate.GetTranslationForRelation(availableTranslations,sourceRelations)
    ...
    val ontologyElements: RDD[(String, String)] = sourceClassesWithBestTranslation.union(relationsWithTranslation)
    val sor = new SourceOntologyReconstruction()
//    val translatedSourceOntology = sor.ReconstructOntology(sOntology,sourceClassesWithBestTranslation).persist(StorageLevel.MEMORY_AND_DISK)
    val translatedSourceOntology = sor.ReconstructOntology(sOntology,ontologyElements).persist(StorageLevel.MEMORY_AND_DISK)
    ...
    val translatedSourceOntologyRelationTriples = translatedSourceOntology.filter(x => listOfSourcePredicatesBroadcasting.value.contains(x._2) && x._2 != "subClassOf" && x._2 != "disjointWith" && x._3 != "Class")
    ...
    var monolingualTriplesForEnrichment: RDD[(String, String, String)] = m.GetTriplesToBeEnriched(translatedSourceOntologyClassTriples, targetClassesWithoutURIs,listOfMatchedClasses)//.cache()
